...
function test_gx_git_config_aliases_help() {
    ...
    assert equals "${count_lines}" "25" $LINENO

    assert first-line   "$(gx_git_config_aliases_help)"   "${TEST_C_WHITE}  # Git aliases${TEST_F_RESET} :" $LINENO
    assert last-line    "$(gx_git_config_aliases_help)"   "      others | git \033[96mpr\033[0m \033[90m...................\033[0m Generates the url to compare and create a PR with the current branch" $LINENO
}
